...
class FeelingLocalDatasource {
    ...
    func addFeeling(_ feeling: FeelingDataModel) throws {
        guard let entitiy = dbm.add(entity: .Feeling) as? Feeling else {throw(URLError(.badURL))}
        entitiy.message = feeling.message
        entitiy.date = feeling.date
        try dbm.save()
    }
    func addActivity(_ feeling: FeelingDataModel) throws {
        guard let entitiy = dbm.add(entity: .Activity) as? Activity else {throw(URLError(.badURL))}
    ...
    func fetchFeelings() throws {
        guard var fetchResutl = try dbm.fetch(entity: .Feeling) as? [Feeling] else{return}
    ...
    func fetchActivies() throws {
        let fetchResutl = try dbm.fetch(entity: .Activity) as? [Activity]
    }
}
